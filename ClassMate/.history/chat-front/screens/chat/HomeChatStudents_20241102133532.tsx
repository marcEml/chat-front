...
const HomeChatStudents = (props: any) => {
  ...
  const className = UserClass.class.name;
  const ClassId = UserClass.class.id;
  const [UserName, SetUserName] = useState("");
  const [UserId, setUserId] = useState<any>();
  const [messageList, SetMessageList] = useState(UserClass.class.messages);
  const [image, setImage] = React.useState<any>(null);

  const socket = io("http://192.168.1.29:2880/");
...
export default HomeChatStudents;
